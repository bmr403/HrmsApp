...
jQuery( document ).ajaxError(function() {
            ...
            jQuery.each(response, function(key, value) {
                jQuery.each(value, function(k, v) {
					...
					if (projectResourceIsActive == false)
					{
						...
						{
							projectsArray[projectMasterId] = projectCode+'_'+projectResourceInActiveFrom;
						}
					}
					else
					{
    ...
    jQuery(window).load(function() {
		...
		activeStatus = localStorage.getItem('activeStatus');
		
		
		if ((timesheetApprovedStatus == "SUBMITTED") || (timesheetApprovedStatus == "APPROVED") || (activeStatus == "0"))
		{
    ...
    jQuery('#addProject_button').on('click', function() {
		...
		projectInfo += "<td><input type=\"text\" class=\"form-control\" style=\"width: 50px;\" name=\"total_" + rowCount + "\" id=\"total_" + rowCount + "\"/></td>";
        projectInfo += "</tr>";
		...
		jQuery("#project_" + rowCount + "").change(function () {
			...
			jQuery.each(projectsArray, function(projectIndex, projectValue) {
				...
				if (projectValueArray.length >= 1)
				{
					...
					{
						var inActvieDateStr = '';
						inActvieDateStr = projectValueArray[1];	
						var inActiveDate;
						
						if (!(typeof inActvieDateStr == 'undefined'))
						{
    ...
    jQuery.ajax({
        ...
        success: function(response) {
			...
			else
			{
				...
				var activeStatus = response.data[0].activeStatus;
            ...
            jQuery.each(response.data, function(index, value) {
                ...
                //if ((!(typeof inActiveFrom == 'undefined')) || (!(inActiveFrom == '')))
				if (inActiveFrom)
				{
